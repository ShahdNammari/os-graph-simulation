...
static void child_run(int write_fd, int src, int dst, const char *filename) {
    ...
    for (int i = 0; i < res->path_len; i++) {
        ...
        /* special message: does this node have a self-loop? */
...
int main(int argc, char *argv[]) {
    ...
    InitWindow(SCREEN_W, SCREEN_H, "Graph Simulation - Milestone 5");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        /* ── poll pipes — read ALL available messages per frame ── */
        if (simulation_started) {
            for (int i = 0; i < num_travelers; i++) {
                ...
                PipeMsg msg;
                ssize_t r;
                while ((r = read(travelers[i].pipe_fd, &msg, sizeof(msg)))
                       == (ssize_t)sizeof(msg)) {

                    if (msg.kind == MSG_KIND_SPECIAL) {
                        /* special message — printed separately, does not affect animation */
